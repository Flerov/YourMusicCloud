...
def get_config(config_file):
    try:
        ...
        for line in yaml.dump(
            default_conf["spotify-spotdl"], default_flow_style=False
        ).split("\n"):
            if line.strip():
                log.info(line.strip())
        log.info(
    ...
    return cfg["spotify-downloader"]  # return cfg["spotify-spotdl"]
...
def override_config(config_file, parser, raw_args=None):
    ...
    config = merge(default_conf["spotify-spotdl"], get_config(config_file))
...
def get_arguments(raw_args=None, to_group=True, to_merge=True):
    ...
    if to_merge:
        ...
        config = default_conf["spotify-spotdl"]
